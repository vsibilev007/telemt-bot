...
def users_to_csv(users: list, server_name: str) -> bytes:
    ...
    writer.writerow([
        "username", "current_connections", "total_octets", "total_octets_human",
        "active_unique_ips", "recent_unique_ips",
        "max_tcp_conns", "max_unique_ips", "data_quota_bytes",
        "expiration_rfc3339", "server",
    ])
    for u in users:
        writer.writerow([
            u.get("username", ""),
...
def users_to_xlsx(users: list, server_name: str, traffic_deltas: list[dict] | None = None) -> bytes:
    ...
    headers = [
        "Пользователь", "Соединений", "Трафик всего", "Трафик за 7д",
        "Активных IP", "Недавних IP",
        "Max TCP", "Max IP", "Квота",
        "Истекает", "Сервер",
    ]

    # Индекс дельт по username
    ...
    for row_idx, u in enumerate(users, 2):
        ...
        row_data = [
            ...
            u.get("max_tcp_conns", ""),
            u.get("max_unique_ips", ""),
            _fmt_bytes(u.get("data_quota_bytes", 0)) if u.get("data_quota_bytes") else "",
            exp[:10] if exp else "",
            server_name,
        ]
        ...
        for col_idx, val in enumerate(row_data, 1):
            ...
            # Подсветка истёкших
            if exp and exp[:10] < datetime.now(timezone.utc).strftime("%Y-%m-%d"):
                if col_idx == 10:
                    cell.fill = red_fill
